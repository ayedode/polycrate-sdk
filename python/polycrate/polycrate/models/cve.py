...
T = TypeVar("T", bound="CVE")


@_attrs_define
class CVE:
    """Full serializer for CVE detail view.

    Attributes:
        id (UUID):
        created (CVECreated):
        organization (CVEOrganizationType0 | None):
        workspace (CVEWorkspaceType0 | None):
        ...
        last_action_run (CVELastActionRunType0 | None):
        deleted_by_user (CVEDeletedByUserType0 | None):
        ...
        title (str | Unset): Short human-readable title/summary
        description (str | Unset): Full vulnerability description
        published_at (datetime.datetime | None | Unset): When the CVE was published upstream
        modified_at (datetime.datetime | None | Unset): When the CVE was last modified upstream
        cvss_score (None | str | Unset): CVSS base score (0.0 - 10.0)
        cvss_vector (None | str | Unset): CVSS vector string, e.g. CVSS:3.1/AV:N/AC:L/...
        severity (Severity401Enum | Unset): * `critical` - Critical
            * `high` - High
            * `medium` - Medium
            * `low` - Low
            * `none` - None
            * `unknown` - Unknown
        status (StatusA85Enum | Unset): * `open` - Open
            * `disputed` - Disputed
            * `rejected` - Rejected
            * `reserved` - Reserved
    ...
    cvss_score: None | str | Unset = UNSET
    cvss_vector: None | str | Unset = UNSET
    severity: Severity401Enum | Unset = UNSET
    status: StatusA85Enum | Unset = UNSET
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if isinstance(self.organization, CVEOrganizationType0):
            organization = self.organization.to_dict()
        else:
            organization = self.organization

        workspace: dict[str, Any] | None
        if isinstance(self.workspace, CVEWorkspaceType0):
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        created = CVECreated.from_dict(d.pop("created"))

        def _parse_organization(data: object) -> CVEOrganizationType0 | None:
            if data is None:
                return data
            try:
                if not isinstance(data, dict):
                    raise TypeError()
                organization_type_0 = CVEOrganizationType0.from_dict(data)

                return organization_type_0
            except (TypeError, ValueError, AttributeError, KeyError):
                pass
            return cast(CVEOrganizationType0 | None, data)

        organization = _parse_organization(d.pop("organization"))

        def _parse_workspace(data: object) -> CVEWorkspaceType0 | None:
            if data is None:
                return data
            try:
                if not isinstance(data, dict):
                    raise TypeError()
                workspace_type_0 = CVEWorkspaceType0.from_dict(data)

                return workspace_type_0
            except (TypeError, ValueError, AttributeError, KeyError):
                pass
            return cast(CVEWorkspaceType0 | None, data)
        ...
        def _parse_last_action_run(data: object) -> CVELastActionRunType0 | None:
            if data is None:
                return data
            try:
                if not isinstance(data, dict):
                    raise TypeError()
                last_action_run_type_0 = CVELastActionRunType0.from_dict(data)

                return last_action_run_type_0
            except (TypeError, ValueError, AttributeError, KeyError):
                pass
            return cast(CVELastActionRunType0 | None, data)

        last_action_run = _parse_last_action_run(d.pop("last_action_run"))

        def _parse_deleted_by_user(data: object) -> CVEDeletedByUserType0 | None:
            if data is None:
                return data
            try:
                if not isinstance(data, dict):
                    raise TypeError()
                deleted_by_user_type_0 = CVEDeletedByUserType0.from_dict(data)

                return deleted_by_user_type_0
            except (TypeError, ValueError, AttributeError, KeyError):
                pass
            return cast(CVEDeletedByUserType0 | None, data)
        ...
        def _parse_cvss_score(data: object) -> None | str | Unset:
            if data is None:
                return data
            if isinstance(data, Unset):
                return data
        ...
        severity: Severity401Enum | Unset
        if isinstance(_severity, Unset):
            severity = UNSET
        else:
            severity = check_severity_401_enum(_severity)

        _status = d.pop("status", UNSET)
        status: StatusA85Enum | Unset
        if isinstance(_status, Unset):
            status = UNSET
        else:
            status = check_status_a85_enum(_status)
