...
T = TypeVar("T", bound="PatchedBackupScheduleRequest")


@_attrs_define
class PatchedBackupScheduleRequest:
    ...
    Siehe: .specs/0.13.0/backupschedule-name-fix.md
        ...
        Attributes:
            name (str | Unset): BackupSchedule name (required, must match regex ^[a-z0-9]+(?:-[a-z0-9]+)*$)
    ...
    def to_multipart(self) -> types.RequestFiles:
        ...
        if not isinstance(self.schedule_cron, Unset):
            if isinstance(self.schedule_cron, str):
                files.append(("schedule_cron", (None, str(self.schedule_cron).encode(), "text/plain")))
            else:
                files.append(("schedule_cron", (None, str(self.schedule_cron).encode(), "text/plain")))

        if not isinstance(self.paused, Unset):
            files.append(("paused", (None, str(self.paused).encode(), "text/plain")))
        ...
        if not isinstance(self.last_backup_at, Unset):
            if isinstance(self.last_backup_at, datetime.datetime):
                files.append(("last_backup_at", (None, self.last_backup_at.isoformat().encode(), "text/plain")))
            else:
                files.append(("last_backup_at", (None, str(self.last_backup_at).encode(), "text/plain")))

        if not isinstance(self.total_backups, Unset):
            files.append(("total_backups", (None, str(self.total_backups).encode(), "text/plain")))
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        patched_backup_schedule_request = cls(
            name=name,
        ...
        patched_backup_schedule_request.additional_properties = d
        return patched_backup_schedule_request
